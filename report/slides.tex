...
\author{Nguyễn Trường Giang \and Đinh Hải Phong}
\institute{Đại học Bách khoa Hà Nội --- Khoa Toán Tin\\ \textit{Học phần: Hệ hỗ trợ quyết định}}
\date{Tháng 6, 2025}
...
\begin{frame}{Lõi dự báo: RevIN + N-BEATS}
  ...
  Siêu tham số tối ưu bằng \textbf{Optuna} (100 trial): lookback 24, hidden 512,
  2 stack $\times$ 2 block, 3 lớp.
\end{frame}
...
\begin{frame}{Tự cài đặt \& so sánh 5 kiến trúc}
  Cùng giao thức (cấu hình, pinball loss, 3 hạt giống) --- chỉ khác \emph{phần lõi}:
  \vspace{0.2cm}
  \begin{columns}[c]
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{lrr}
        ...
        NHITS          & 7.722  & 2,2\% \\
        MLP            & 6.203  & \textbf{1,8\%} \\
        \textbf{N-BEATS} & \textbf{6.234} & 1,8\% \\
        \bottomrule
      \end{tabular}
      \vspace{0.2cm}
      \par{\scriptsize (test 24 tháng, giao thức chung)}
    \end{column}
    \begin{column}{0.54\textwidth}
      \includegraphics[width=\linewidth]{cmp1_mae_mape}
    \end{column}
  \end{columns}
  \vspace{0.15cm}
  {\small \textbf{N-BEATS và MLP mạnh nhất, gần như ngang nhau}; DLinear/TSMixer
  (thiết kế cho dài hạn/đa biến) kém hơn trên bài toán đơn biến này.}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.36\textwidth}
      \begin{block}{N-BEATS tinh chỉnh}
        \begin{itemize}
          \item MAE = \textbf{5.048}
          \item MAPE = \textbf{1,52\%}
          \item RMSE = 5.787
        \end{itemize}
      \end{block}
      Vượt trội gần \textbf{16 lần} so với Naive (MAE 80.045).
    \end{column}
...
\begin{frame}{Mô hình Newsvendor: phân vị tối ưu}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      $\Rightarrow$ P88 $\approx 321.700$ $\Rightarrow$ đặt thêm \textbf{221.700}.}
    \end{column}
    \begin{column}{0.5\textwidth}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item Mới dùng đơn biến (chưa khai thác biến ngoại sinh).
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}{Kết luận}
  \begin{enumerate}
    \item \textbf{Dự báo đa phân vị} RevIN + N-BEATS tự triển khai, đạt MAPE
          \textbf{1,52\%} --- vượt baseline $\sim$16 lần; so sánh khách quan 5 kiến trúc.
