...
\begin{frame}{Ai dùng \& cần quyết định gì?}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item Lượng đặt, cỡ lô, điểm đặt lại, tồn kho an toàn.
        \item Thống nhất trong mô hình \textbf{$(r,q)$} (Newsvendor + EOQ).
      \end{itemize}
    \end{column}
  \end{columns}
  \vfill
...
\begin{frame}{Kiến trúc hệ thống}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=of dist, fill=orange!18] (nv) {Tồn kho\\$(r,q)$};
...
\begin{frame}{Mô hình tồn kho $(r,q)$: tỷ lệ tới hạn}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      Mức phục vụ tối ưu = \textbf{tỷ lệ tới hạn}; mức chuẩn bị = phân vị tại đó:
      \[ \CR = \frac{\Cu}{\Cu + \Co}, \quad Q^* = F^{-1}(\CR) \]
      \vspace{-0.2cm}
      \begin{itemize}\small
        \item Lãi cao ($\Cu$ lớn) $\Rightarrow \CR\!\to\!1 \Rightarrow$ phục vụ cao (P90+).
        \item $z=\Phi^{-1}(\CR) \Rightarrow$ SS $= z\sigma\sqrt{L}$, điểm đặt lại $r$; EOQ cho cỡ lô.
      \end{itemize}
      \vspace{0.15cm}
...
\begin{frame}{Kết luận}
  \begin{enumerate}
    ...
    \item \textbf{Kết nối dự báo $\rightarrow$ quyết định} qua mô hình $(r,q)$ (Newsvendor + EOQ):
          khuyến nghị đặt hàng có cơ sở kinh tế, giải thích được.
