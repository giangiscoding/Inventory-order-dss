...
% ==================== BÌA ====================
\definecolor{bkblue}{HTML}{1A3A8F}
\begin{titlepage}
\begin{tikzpicture}[remember picture, overlay]
  ...
  \draw[bkblue, line width=1pt] ($(current page.north west)+(0.82cm,-0.82cm)$) rectangle ($(current page.north west)+(1.53cm,-1.53cm)$);
  \draw[bkblue, line width=1pt] ($(current page.north east)+(-0.82cm,-0.82cm)$) rectangle ($(current page.north east)+(-1.53cm,-1.53cm)$);
  \draw[bkblue, line width=1pt] ($(current page.south west)+(0.82cm,0.82cm)$) rectangle ($(current page.south west)+(1.53cm,1.53cm)$);
  \draw[bkblue, line width=1pt] ($(current page.south east)+(-0.82cm,0.82cm)$) rectangle ($(current page.south east)+(-1.53cm,1.53cm)$);
\end{tikzpicture}
\begin{center}
\vspace*{0.4cm}

{\large \textbf{ĐẠI HỌC BÁCH KHOA HÀ NỘI}}\\[3pt]
{\large \textbf{KHOA TOÁN - TIN}}\\[2pt]
{\textbf{---o0o---}}

\vspace{0.7cm}
\includegraphics[height=4.3cm]{figures/logo.png}

\vspace{0.7cm}
{\LARGE \bfseries BÁO CÁO MÔN}\\[0.35cm]
{\LARGE \bfseries HỆ HỖ TRỢ QUYẾT ĐỊNH}

\vfill

{\large \bfseries\itshape ĐỀ TÀI}\\[0.4cm]
\rule{0.85\linewidth}{0.4pt}\\[0.4cm]
{\large \bfseries HỆ HỖ TRỢ QUYẾT ĐỊNH ĐẶT HÀNG DỰA TRÊN\\[2pt]
DỰ BÁO CHUỖI THỜI GIAN ĐA PHÂN VỊ VÀ\\[2pt]
TỐI ƯU HÓA TỒN KHO}\\[0.4cm]
\rule{0.85\linewidth}{0.4pt}

\vfill

{\large \textbf{GVHD:} Trần Ngọc Thăng, Lê Hải Hà}

\vspace{0.55cm}
{\large \textbf{Nhóm sinh viên thực hiện: Lớp 169317}}

\vspace{0.3cm}
{\renewcommand{\arraystretch}{1.35}
\begin{tabular}{ll}
\textit{Họ và tên} \hspace{2.5cm} & \textit{MSSV} \\
\hline
Nguyễn Trường Giang & 20227044 \\
...
Báo cáo gồm 6 chương: Chương 2 trình bày cơ sở lý thuyết (các mô hình dự báo và tối ưu tồn kho); Chương 3 mô tả dữ liệu, tiền xử lý và phương pháp; Chương 4 trình bày kết quả thực nghiệm, \textit{so sánh các mô hình} và khuyến nghị quyết định; Chương 5 nêu hạn chế và định hướng phát triển; Chương 6 trình bày phân công công việc.
...
Nhóm gồm hai thành viên, phân công và đóng góp như Bảng~\ref{tab:phancong}. Minh chứng cụ thể (commit, notebook, hình/bảng trong báo cáo) được liệt kê ở cột cuối.

\begin{table}[H]
\centering
\caption{Bảng phân công công việc và đóng góp cá nhân}
\label{tab:phancong}
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.20\linewidth}|>{\raggedright\arraybackslash}p{0.40\linewidth}|c|>{\raggedright\arraybackslash}p{0.20\linewidth}|}
\hline
\rowcolor{gray!18}
\textbf{Thành viên} & \textbf{Công việc chính} & \textbf{Đóng góp} & \textbf{Minh chứng} \\
\hline
Nguyễn Trường Giang (20227044)
  & Xây dựng mô hình dự báo (RevIN, N-BEATS, các mô hình so sánh), huấn luyện, tối ưu Optuna, so sánh kiến trúc, ứng dụng web.
  & 50\%
  & \texttt{src/}, \texttt{app/}, hình 4.1-4.4, bảng 4.2 \\
\hline
Đinh Hải Phong (20210983)
  & Thu thập \& tiền xử lý dữ liệu, phân tích khám phá (EDA), mô hình tối ưu tồn kho (Newsvendor, EOQ), viết và hoàn thiện báo cáo.
  & 50\%
  & \texttt{Data/}, EDA (Ch.~3), \texttt{inventory.py}, báo cáo \\
\hline
\end{tabular}
\end{table}

\textit{Ghi chú: tỉ lệ đóng góp và phân công ở trên là đề xuất, nhóm điều chỉnh theo thực tế trước khi nộp.}
...
Nhóm có sử dụng công cụ AI hỗ trợ trong quá trình thực hiện, tuân thủ quy định của học phần. Chi tiết ở Bảng~\ref{tab:ai}. Nhóm \textbf{chịu trách nhiệm hoàn toàn} về tính chính xác của nội dung, kết quả phân tích và khuyến nghị; mọi kết quả số đều được kiểm chứng bằng cách chạy lại mã nguồn.
...
Phần \textbf{không} dùng AI tạo tự động: thiết kế bài toán, lựa chọn phương pháp, diễn giải kết quả và các khuyến nghị quyết định - đây là phần do nhóm tự thực hiện dựa trên dữ liệu và kết quả mô hình.

% ==================== PHỤ LỤC D: TỰ CHẤM ====================
\chapter{Bảng tự chấm điểm}

\begin{table}[H]
\centering
\caption{Bảng nhóm tự đánh giá theo tiêu chí chấm điểm}
\label{tab:selfgrade}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|>{\raggedright\arraybackslash}p{0.46\linewidth}|c|c|}
\hline
\rowcolor{gray!18}
...
\hline
\rowcolor{gray!18}
 & \textbf{Tổng cộng} & \textbf{100} & \textbf{93} \\
\hline
\end{tabular}
\end{table}

\textit{Ghi chú: điểm tự chấm là đề xuất tham khảo, không thay thế điểm của giảng viên.}
