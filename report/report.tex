...
Trên tập kiểm tra (2024), mô hình đạt \textbf{MAE = 3.254, MAPE = 0,97\%}, vượt trội gần \textbf{24 lần} so với phương pháp cơ sở Naive (MAE = 79.188, MAPE = 22,76\%). Để kiểm chứng lựa chọn kiến trúc, chúng tôi \textbf{tự cài đặt và so sánh thêm bốn kiến trúc dự báo phân vị khác} (MLP, DLinear, TSMixer, NHITS) dưới cùng một giao thức huấn luyện: N-BEATS và NHITS cho kết quả tốt và ổn định nhất. Về quyết định tồn kho, kết quả phân vị được đưa vào \textbf{mô hình Newsvendor} - tỷ lệ tới hạn $\CR = \Cu/(\Cu + \Co)$ xác định trực tiếp mức phân vị tối ưu cần chuẩn bị, mang ý nghĩa kinh tế rõ ràng mà dự báo điểm không thể hiện được. Mô hình \textbf{EOQ} bổ sung khuyến nghị cỡ lô cho vận hành dài hạn.
...
\hline
Kết quả quan trọng nhất? & MAPE 0,97\% (test); N-BEATS tốt nhất khi so sánh & Ch.~4 \\
\hline
...
\hline
Chuẩn hóa thang đo & Chia chuỗi \texttt{Quantity} cho trung bình tập train ($s = 351{,}189$) để ổn định gradient; RevIN sau đó chuẩn hóa từng cửa sổ ở cấp instance (mục~\ref{sec:revin_theory_ref}). \\
\hline
...
\hline
Naive          & 79.188 & 99.343 & 22,76 & -- & 0 \\
Seasonal Naive & 71.495 & 96.009 & 23,37 & -- & 0 \\
\hline
TSMixer & 15.717 $\pm$ 2.149 & 23.975 & 4,36 & 5.283 & 4.294 \\
DLinear & 11.853 $\pm$ 2.116 & 18.973 & 3,41 & 4.535 & 350 \\
MLP     & 6.232 $\pm$ 417 & 7.521 & 1,87 & 1.866 & 139.783 \\
NHITS   & 5.713 $\pm$ 72 & 7.510 & 1,69 & 1.665 & 422.463 \\
\rowcolor{gray!18}
\textbf{N-BEATS} & \textbf{5.283 $\pm$ 324} & \textbf{6.897} & \textbf{1,57} & \textbf{1.585} & 583.804 \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{N-BEATS tốt nhất} ở mọi chỉ số điểm (MAE, RMSE, MAPE) lẫn pinball loss, theo sát là \textbf{NHITS}. Hai kiến trúc dùng cơ chế \textit{doubly-residual} và phân tách tín hiệu tỏ ra phù hợp nhất với chuỗi này.
  \item \textbf{NHITS ổn định nhất} (độ lệch chuẩn MAE chỉ $\pm$72, nhỏ hơn N-BEATS $\pm$324) trong khi dùng ít hơn $\sim$28\% tham số - một lựa chọn đáng cân nhắc nếu ưu tiên tính ổn định.
  \item \textbf{DLinear và TSMixer kém hơn rõ rệt} (MAE gấp 2-3 lần). Đây là các mô hình được thiết kế cho dự báo \textit{dài hạn, đa biến}; trên bài toán đơn biến horizon ngắn ($H=1$) chúng mất lợi thế - đặc biệt TSMixer khi phần feature-mixing thoái hóa do chỉ có một kênh.
  \item \textbf{Cả năm mô hình học sâu đều vượt xa} hai baseline (MAE giảm 5-15 lần), khẳng định giá trị của học sâu cho bài toán này.
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{Dự báo trung vị (P50) của từng mô hình so với thực tế trên tập test 2024. N-BEATS, NHITS, MLP bám sát; DLinear và TSMixer lệch nhiều hơn ở các đỉnh (tháng 3, tháng 11).}
  \label{fig:cmp_forecast}
\end{figure}

\textbf{Kết luận so sánh:} kết quả này biện minh cho việc chọn N-BEATS làm mô hình triển khai chính. Sau khi tối ưu siêu tham số bằng Optuna (mục~\ref{sec:best_params}), N-BEATS đạt MAE = 3.254 (thấp hơn cả mức 5.283 dưới giao thức chung), củng cố thêm lựa chọn này.
...
Hình~\ref{fig:cmp_calib} so sánh calibration giữa các kiến trúc. Một phát hiện đáng chú ý: \textbf{độ chính xác điểm và độ hiệu chỉnh không trùng nhau}. MLP - tuy có MAE cao hơn - lại cho đường calibration \textit{gần đường lý tưởng nhất}, trong khi N-BEATS có xu hướng \textit{bảo thủ} (bao phủ thấp ở phân vị cao). Điều này gợi ý rằng nếu ưu tiên độ tin cậy của khoảng phân vị (quan trọng cho quyết định tồn kho theo $\CR$), cần cân nhắc cả calibration chứ không chỉ MAE; đây là một hướng kết hợp tiềm năng (vd. conformal prediction, mục~\ref{sec:future}).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\linewidth]{figures/cmp4_calibration.png}
  \caption{Calibration của các kiến trúc trên tập test. MLP bám đường lý tưởng tốt nhất; N-BEATS bảo thủ (undercover) ở phân vị cao. Tập test nhỏ (n=12) nên đánh giá chỉ mang tính tham khảo.}
  \label{fig:cmp_calib}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{Dùng N-BEATS (hoặc NHITS) làm lõi dự báo.} So sánh khách quan (mục~\ref{sec:comparison}) cho thấy hai kiến trúc này vượt trội; NHITS là lựa chọn thay thế tốt nếu ưu tiên tính ổn định và tiết kiệm tham số.
...
\begin{enumerate}
  ...
  \item \textbf{Phạm vi so sánh mô hình:} Việc so sánh dùng \textit{cùng một giao thức} (cùng cấu hình) nên có thể chưa khai thác hết tiềm năng riêng của từng kiến trúc (DLinear, TSMixer vốn mạnh ở dự báo dài hạn/đa biến). Ngoài ra, kết quả calibration cho thấy mô hình có MAE thấp nhất (N-BEATS) không phải mô hình hiệu chỉnh tốt nhất - cần cân nhắc cả hai tiêu chí khi triển khai.
\end{enumerate}
