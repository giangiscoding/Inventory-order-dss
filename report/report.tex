...
Trên tập kiểm tra 24 tháng (2023-2024), mô hình đạt \textbf{MAE = 5.048, MAPE = 1,52\%}, vượt trội gần \textbf{16 lần} so với phương pháp cơ sở Naive (MAE = 80.045, MAPE = 23,59\%). Để kiểm chứng lựa chọn kiến trúc, em đã \textbf{tự cài đặt và so sánh thêm bốn kiến trúc dự báo phân vị khác} (MLP, DLinear, TSMixer, NHITS) dưới cùng một giao thức huấn luyện, trong đó DLinear và TSMixer chạy \textbf{đa biến} với 6 biến ngoại sinh tương quan cao nhất. N-BEATS đạt pinball loss tốt nhất, theo sát là \textbf{TSMixer đa biến} (cực gọn nhẹ); sau khi tinh chỉnh siêu tham số, N-BEATS được chọn triển khai. Về quyết định tồn kho, kết quả phân vị được đưa vào \textbf{mô hình Newsvendor} - tỷ lệ tới hạn $\CR = \Cu/(\Cu + \Co)$ xác định trực tiếp mức phân vị tối ưu cần chuẩn bị, mang ý nghĩa kinh tế rõ ràng mà dự báo điểm không thể hiện được. Mô hình \textbf{EOQ} bổ sung khuyến nghị cỡ lô cho vận hành dài hạn.
...
Phần này so sánh năm kiến trúc dự báo phân vị tự cài đặt cùng hai baseline điểm, dưới \textit{cùng giao thức} ở mục~\ref{sec:protocol} (cùng cấu hình, 3 hạt giống, trung bình $\pm$ độ lệch chuẩn). Trong đó MLP, NHITS, N-BEATS chạy \textbf{đơn biến}; DLinear và TSMixer chạy \textbf{đa biến} (\texttt{Quantity} + 6 biến ngoại sinh). Mục tiêu là kiểm chứng lựa chọn N-BEATS và đánh giá lợi ích của thông tin ngoại sinh.

\begin{table}[H]
\centering
\caption{So sánh hiệu suất các mô hình trên tập test 24 tháng (trung bình $\pm$ độ lệch chuẩn qua 3 hạt giống). Mô hình học sâu dùng cùng giao thức; (đa biến) = dùng \texttt{Quantity} + 6 ngoại sinh.}
...
\hline
DLinear (đa biến) & 12.598 $\pm$ 6.602 & 15.922 & 3,90 & 3.841 & 2.366 \\
NHITS   & 7.722 $\pm$ 901 & 9.279 & 2,22 & 2.235 & 422.463 \\
MLP     & 6.203 $\pm$ 515 & 7.770 & \textbf{1,76} & 2.026 & 139.783 \\
TSMixer (đa biến) & 6.339 $\pm$ \textbf{240} & 8.352 & 1,88 & 1.919 & 14.608 \\
\rowcolor{gray!18}
...
\textbf{Nhận xét chính} (tập test 24 tháng):
\begin{itemize}
  \item \textbf{N-BEATS tốt nhất theo pinball loss (1.868) và RMSE (7.664)}; \textbf{TSMixer đa biến bám rất sát} (pinball 1.919) và \textbf{ổn định nhất} (độ lệch chuẩn MAE chỉ $\pm$240). MLP nhỉnh nhất về MAE (6.203) và MAPE (1,76\%). Bốn mô hình tốt nhất gần như ngang nhau, khác biệt nằm trong sai số ngẫu nhiên.
  \item \textbf{TSMixer cải thiện vượt bậc nhờ đa biến.} Khi được cho dùng 6 biến ngoại sinh tương quan cao (IPI, Imports\ldots), MAE của TSMixer giảm từ $\sim$22.000 (chế độ đơn biến) xuống còn 6.339 - phần feature-mixing phát huy và thông tin ngoại sinh bổ sung tín hiệu hữu ích. Đáng chú ý, TSMixer đạt kết quả này với \textbf{chỉ $\sim$14.600 tham số} (nhỏ hơn N-BEATS $\sim$40 lần) - mô hình hiệu quả tham số nhất.
  \item \textbf{DLinear (đa biến) cải thiện nhưng còn yếu và bất ổn} (MAE 12.598, độ lệch chuẩn $\pm$6.602 rất lớn): lõi tuyến tính gọn chưa khai thác tốt quan hệ phi tuyến giữa các biến.
  \item \textbf{Cả năm mô hình học sâu đều vượt xa} hai baseline (MAE giảm $\sim$6-13 lần). Thông tin ngoại sinh giúp một kiến trúc đa biến (TSMixer) cạnh tranh ngang hàng với mô hình đơn biến tốt nhất.
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{Chi tiết các mô hình học sâu: MAE, pinball loss và số tham số. TSMixer (đa biến) chính xác ngang N-BEATS/MLP nhưng cực gọn; DLinear bất ổn (thanh sai số lớn).}
  \label{fig:cmp_deep}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Dự báo trung vị (P50) của từng mô hình so với thực tế trên tập test 2023-2024. N-BEATS, MLP, TSMixer (đa biến) bám sát; DLinear lệch nhiều hơn ở các đỉnh và đáy.}
  \label{fig:cmp_forecast}
\end{figure}

\textbf{Kết luận so sánh:} bốn mô hình tốt nhất (N-BEATS, TSMixer đa biến, MLP, NHITS) gần như ngang nhau. Chúng tôi chọn \textbf{N-BEATS} làm mô hình triển khai vì: (i) dẫn đầu pinball loss - thước đo chính cho dự báo phân vị; (ii) là mô hình \textit{đơn biến}, không phụ thuộc việc thu thập biến ngoại sinh khi vận hành; (iii) còn dư địa cải thiện qua tinh chỉnh - sau khi tối ưu siêu tham số bằng Optuna (mục~\ref{sec:best_params}), N-BEATS đạt MAE = 5.048, thấp hơn mức 6.234 dưới giao thức chung và tốt hơn mọi mô hình khác. \textbf{TSMixer đa biến} là hướng thay thế đáng chú ý: chính xác tương đương, cực gọn, và cho thấy thông tin ngoại sinh có giá trị nếu được khai thác đúng cách.
...
\begin{enumerate}
  ...
  \item \textbf{Phạm vi so sánh mô hình:} Việc so sánh dùng \textit{cùng một giao thức} (cùng cấu hình) nên có thể chưa khai thác hết tiềm năng riêng của từng kiến trúc; các mô hình đa biến chỉ dùng 6 ngoại sinh chọn theo tương quan tuyến tính, chưa thử các tập biến hay độ trễ khác. Ngoài ra, bốn mô hình tốt nhất gần như ngang nhau về độ chính xác, cho thấy không nên chỉ dựa vào một chỉ số - cần cân nhắc cả độ chính xác lẫn calibration khi chọn mô hình triển khai.
\end{enumerate}
